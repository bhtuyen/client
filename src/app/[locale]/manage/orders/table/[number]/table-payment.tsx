...
export default function TablePayment({ number, isPaid }: { number: string; isPaid: boolean }) {
  ...
  const description = useMemo(() => {
    return `${envConfig.NEXT_PUBLIC_CODE}${number}`;
  }, [number]);
        ...
        <div className='flex'>
          <div className='flex-1 flex flex-col gap-4 items-center border border-r-0 rounded-sm p-4'>
            ...
            <div className='flex flex-col items-center gap-2'>
              <div>
                <TImage
                  src={`https://qr.sepay.vn/img?bank=${envConfig.NEXT_PUBLIC_BANK}&acc=${envConfig.NEXT_PUBLIC_ACCOUNT_NUMBER}&template=compact&amount=${amount}&des=${description}`}
                  width={400}
                  height={400}
                  alt='QR code'
                  className=''
          ...
          <div className='flex-1 flex flex-col gap-4 items-center border rounded-sm p-4'>
            ...
            <div className='flex items-center gap-2'>
              <TImage
                src={`/${envConfig.NEXT_PUBLIC_BANK.toLowerCase()}-icon.png`}
                width={40}
                height={40}
                alt={envConfig.NEXT_PUBLIC_BANK.toLowerCase()}
              />
              <p className='font-bold'>{tInfo(envConfig.NEXT_PUBLIC_BANK.toLowerCase() as TMessageKeys<'t-info'>)}</p>
            </div>
            ...
            <div className='p-4 rounded-sm'>{tInfo('note-for-payment')}</div>
          </div>
        </div>
      </div>
      <div className='flex-1 p-4 pr-2 rounded-sm h-full flex flex-col gap-4'>
            ...
            {orders.map(({ dishSnapshot, quantity }) => (
              <li className='flex justify-between items-center bg-white p-2 rounded-sm border-b' key={dishSnapshot.id}>
                <div className='grid grid-cols-[1fr_auto] items-center text-base flex-[7]'>
                  ...
                  <span className='p-1 rounded-sm text-sm'>x{quantity}</span>
                </div>
